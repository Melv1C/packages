...
export default defineConfig({
  ...
  onSuccess: async () => {
    ...
    copyFileSync(join(process.cwd(), 'src/styles/base.css'), join(process.cwd(), 'dist/base.css'));
    ...
    for (const file of themeFiles) {
      copyFileSync(join(themesDir, file), join(process.cwd(), 'dist/themes', file));
    }

    console.log(`✅ CSS files copied to dist folder (${themeFiles.length} themes)`);
  },
});
